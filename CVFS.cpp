#include<stdio.h>//io file
#include<stdlib.h>//malloc free
#include<string.h>//string
#include<unistd.h>
#include<iostream>//
//#include<io.h>
//#define vale 20 line pryant hey sagle macro aahet.
#define MAXINODE 50 //5...50 ha akda 50 files create kru shkto

#define READ 1 // hey 2 mode/permission aahe
#define WRITE 2

#define MAXFILESIZE 2048 //1024

#define REGULAR 1 //hey file che 2 type aahet
#define SPECIAL 2//hi file aapn support krt nhi
...
UFDT UFDTArr[50];//[MAXINODE]
SUPERBLOCK SUPERBLOCKobj;//superblock cha object tyar kela
PINODE head = NULL;
...
PINODE Get_Inode(char *name)
{
    ...
    if(name == NULL)
       ...
       while(temp != NULL)//ha loop complete linklist la travel kren
       {
...
int CreateFile(char *name, int permission)
{
    ...
    if((name == NULL) || (permission == 0) || (permission > 3))
        return -1;//hya return -1 baddal bollet sir

  if(SUPERBLOCKobj.FreeInode == 0)
  return -2; 

  (SUPERBLOCKobj.FreeInode)--;//hey sentence return -3  nantr ghetle tr
  ...
  while(i<50)//50chya ethe MAXINODE sir...laloop ufdt madhe shodhnya sathi
  {
     if(UFDTArr[i].ptrfiletable == NULL)
         break;
         i++;
...
//vrchya sentence n memory allocate keli
...
//hey khalch intialisation inode sathi
...
int main()
{
    ...
    char command[4][80],str[80],arr[1024];//local variable

    InitialiseSuperBlock();
    CreateDILB();
     //shell----sir
    while(1)//user interaction yethun start
    {
        fflush(stdin);
        strcpy(str,"");
        ...
        else if(count == 2)
        {
            if(strcmp(command[0],"stat") == 0)
            {
                ...
                if(ret == -2)
                printf("ERRPOR : There is nosuch file\n");
                continue;
            }
            else if(strcmp(command[0],"fstat") == 0)
            {
                ...
                if(ret == -1)
                printf("ERRPOR : Incorrect parameters\n");
                if(ret == -2)
                printf("EERROR : There is no such file\n");
                continue;
            }
            else if(strcmp(command[0],"close") == 0)
            {
                ...
                if(ret == -1)
                printf("ERRPOR : There is no such file\n");
                continue;
            }
